...
/**
 * The duel is defined in `scenarios/duel.ts` rather than here, so that the
...
function describeBattle(run: Battle): string {
  return (
    ...
    `contacts=${run.totalContacts} severed=${run.totalSevered} culled=${run.totalCulled} ` +
    `scrap=${(run.ships.discarded / 1000).toFixed(1)}t`
  );
}
...
export function beamVGunScenario(seed = 20260905): ScenarioRun {
  const run = beamVGun(seed);
  return {
    step: () => run.step(),
    checksum: () =>
      checksumDamage(
...
export function columnScenario(seed = 20260905): ScenarioRun {
  const run = column(seed);
  return {
    step: () => run.step(),
    checksum: () =>
      checksumDamage(
